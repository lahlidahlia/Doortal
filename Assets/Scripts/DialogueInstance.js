...
if (curContent) {
	switch (display.pos) {
		...
		case 1:
		var p = 150;
		if (display.align == 1) p = 150;
		var p2 = 20;
		if (display.align == 1) p2 = Screen.width-220;
			GUI.Label(Rect(p,Screen.height-525,500,500),display.img);
...
function ShowWheel () {
GUI.Label(Rect(Screen.width/2-140,Screen.height-400,280,133),"","wheelbase");
		if (display.choices.length>0) {
			if (GUI.Button(Rect(0,Screen.height-400,Screen.width/2,43),display.choices[0].shortText,"r1c1active")) toLoad=display.choices[0].next;	
		} else {
			GUI.Label(Rect(0,Screen.height-400,Screen.width/2,43),"");
		}		
		if (display.choices.length>1) {
			if (GUI.Button(Rect(Screen.width/2,Screen.height-400,Screen.width/2,43),display.choices[1].shortText,"r1c2active")) toLoad=display.choices[1].next;	
		} else {
			GUI.Label(Rect(Screen.width/2,Screen.height-400,Screen.width/2,43),"");
		}
...
function ShowList () {
	GUILayout.BeginArea(Rect(0,0,Screen.width,150),"","box");
